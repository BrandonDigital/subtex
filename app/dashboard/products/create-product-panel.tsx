...
import { createProduct, createBulkDiscount } from "@/server/actions/admin";
...
export function CreateProductPanel({
  ...
  const handleClose = useCallback(() => {
    ...
    setTimeout(() => {
      setIsVisible(false);
      setFormData(defaultProduct); // Reset form
      onClose();
    }, 300);
  }, [onClose]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Stock is now directly on the product, no need to create variants
